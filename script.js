...
// Criar um a lista de elemento vazio
let todoItems = [];
...
item.querySelector("[data-delete]").addEventListener("click", function (e){
    e.preventDefault();
    if(confirm("Ops! tem certeza que você quer remover este item?")){
        itemsList.removeChild(item);
        removeItem(item);
        setLocalStorage(todoItems);
        alertMessage("Este item foi deletado!", "alert-success");
...
document.addEventListener("DOMContentLoaded", () => {
    form.addEventListener("submit", (e) => {
        ...
        if(itemName.length === 0){
            alertMessage("Ei, NÃO pode iniciar uma tarefa vazia!  já pode começar com uma tarefa... ", "alert-danger");
        }
        else{
            ...
            if (currentItemIndex){
                ...
                alertMessage("Item foi atualizado com sucesso!", "alert-message");
            }
        else{
            const itemObj = {
                name: itemName,
            ...
            setLocalStorage(todoItems);
            alertMessage("Aí sim! Novo item foi adicionado!!", "alert-message");
        }
        getList(todoItems);
    }
